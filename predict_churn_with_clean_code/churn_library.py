...
def eda_plot(df: DataFrame, feature: str, kind: str = "hist", **kwargs) -> plt.Figure:
    """
    helper function to plot a specific feature of a dataframe
    input:
            df: pandas dataframe
            feature: feature of the dataframe to be plotted
            kind: type of plot. Choose between histogram ("hist"), bar plot ("bar"). distribution plot ("dist") or heatmap ("heatmap")
            **kwargs: additional key arguments of specific plots according to matplotlib or sns libraries
    output:
    ...
    if kind == "hist":
        ...
        try:
            ...
            logging.error(f'Feature {feature} is categorical. A distribution plot cannot be generated')
            return

    elif kind == "heatmap":
        sns.heatmap(df.corr(), annot=False, cmap='Dark2_r', linewidths = 2, **kwargs)
    else:
        raise ValueError("Please choose plot kind amongst the following options \
...
def perform_eda(df: DataFrame, 
    ...
    for feature in features_to_plot:
        for kind in ['hist', 'bar', 'dist']:
            ...
            try:
                plot.savefig(os.path.join(output_pth,f'{feature}_{kind}.png'))
...
def encoder_helper(df: DataFrame, category_lst: List[str], origin: str, response: str = '') -> DataFrame:
    '''
...
def perform_feature_engineering(input: DataFrame, target: DataFrame or Series, **kwargs) -> Tuple[DataFrame]:
    '''
    input:
    ...
    '''
    return train_test_split(input, target, **kwargs)
...
def classification_report_image(y_train: DataFrame,
    ...
    # Save figure
    model_name = '_'.join(model_name.split(' ')).lower() # appropriate filename
    plt.savefig(os.path.join(output_pth,f'classification_report_{model_name}.png'))
    plt.close() # save memory


def feature_importance_plot(model: object, X_data: DataFrame, output_pth:str = './images', model_name: str = None) -> None:
    '''
    ...
    # Save Figure
    model_name = '_'.join(model_name.split(' ')).lower() # appropriate filename
    plt.savefig(os.path.join(output_pth,f'feature_importance_plot_{model_name}.png'))
    plt.close() # save memory


def save_model(model: object, model_name: str = None, output_pth: str = './models') -> None: 
    """
...
def train_models(X_train: DataFrame, 
    ...
    cv_rfc = GridSearchCV(estimator=rfc, param_grid=param_grid, cv=2, verbose=3)
    ...
    classification_report_image(y_train, y_test, y_train_preds_rf, y_test_preds_rf, 'Random Forest')
    classification_report_image(y_train, y_test, y_train_preds_lr, y_test_preds_lr, 'Linear Regression')
...
if __name__ == '__main__':
    ...
    perform_eda(df, features_to_plot=['Churn','Customer_Age','Marital_Status','Total_Trans_Ct'], output_pth='./images/eda')
    y = df['Churn']
    X = pd.DataFrame()
    cat_columns = [
        'Gender',
    ...
    X_train, X_test, y_train, y_test = perform_feature_engineering(X, y, test_size= 0.3, random_state=42)
